...
- オープンソース
    - RVizやGazebo、Moveltなどの有料ソフトであれば数十万もするような高機能ツールがオープンソースであり、ロボット開発に役立ちます。
...
- コミュニティ
    - ROSプロジェクトやその関連パッケージへのオープンソースによる貢献は世界規模で行われています。ROSコミュニティは小型なマイコン上で動くROSのプロジェクトから自動運転や産業用ロボットと広いです。
    - ROSの国際的な会議「ROSCon」では多くの研究者・開発者が参加し、ROSに関わる最新の成果を発表しています。
